...
class LevelMenu:
    ...
    def draw_choice_buttons(self):
        ...
        first_btn_rect = pygame.Rect(self._window.get_width() // 2 - btn_width // 2, self._window.get_height() // 2 - btn_heigth*2, btn_width, btn_heigth)
        scnd_btn_rect = pygame.Rect(self._window.get_width() // 2 - btn_width // 2, self._window.get_height() // 2 + btn_heigth*2, btn_width, btn_heigth)
        rects.append(first_btn_rect)
        rects.append(scnd_btn_rect)
        if self._stage.get_inactive_weapons():
            third_btn_rect = pygame.Rect(self._window.get_width() // 2 - btn_width // 2, self._window.get_height() // 2, btn_width, btn_heigth)
    ...
    def draw_choice_text(self, buttons):
        ...
        choice1_pos = buttons[0].x + buttons[0].width // 2 - choice1.get_width() // 2, buttons[0].y + buttons[0].height // 2 - choice1.get_height() // 2
        if len(buttons) > 2:
            choice2_pos = buttons[1].x + buttons[1].width // 2 - choice2.get_width() // 2, buttons[1].y + buttons[1].height // 2 - choice2.get_height() // 2
            choice3_pos = buttons[2].x + buttons[2].width // 2 - choice3.get_width() // 2, buttons[2].y + buttons[2].height // 2 - choice3.get_height() // 2
            self._window.blit(choice2, (choice2_pos))
        else:
            choice3_pos = buttons[1].x + buttons[1].width // 2 - choice3.get_width() // 2, buttons[1].y + buttons[1].height // 2 - choice3.get_height() // 2
    ...
    def draw_weapon_text(self, weapon_rects):
        for weapon, rect in weapon_rects:
            if weapon.active:
                ...
                text = self._font.render(f"Add {type(weapon).__name__} to player", True, (255, 255, 255))
            text_pos = rect.x + rect.width // 2 - text.get_width() // 2, rect.y + rect.height // 2 - text.get_height() // 2
            self._window.blit(text, (text_pos))